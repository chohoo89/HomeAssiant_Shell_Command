...
if __name__ == "__main__":     
    ...
    def HA_GOOGLE_SAY(bearer_token, entity_id, message):
        head = {'Content-Type': 'application/json',
                'Authorization': 'Bearer ' + bearer_token}
        data = {"entity_id" : entity_id, "message" : message}

        url = 'http://192.168.0.2:8123/api/services/tts/google_say'
        response = requests.post(url, json=data, headers=head)
        print(message)

    def HA_NAVER_SAY(bearer_token, entity_id, message):
        head = {'Content-Type': 'application/json',
                'Authorization': 'Bearer ' + bearer_token}
        data = {"entity_id" : entity_id, "message" : message}

        url = 'http://192.168.0.2:8123/api/services/tts/naver_premium_say'
        response = requests.post(url, json=data, headers=head)
        print(message)
    ...
    while True:
        ...
        if first_anounce == False and hex_data[6:8] == "bc":
            ...
            HA_NAVER_SAY(bearer_token, entity_id, "엘리베이터를 호출 하였습니다. 현재 엘리베이터는 " + floor_message + "층 입니다.")
            first_anounce = True
            
        
        if hex_data[20:22] == "03":
            HA_NAVER_SAY(bearer_token, entity_id, "엘리베이터가 도착 하였습니다. 안녕히 가세요")
            break
